---
  ...
  分支设计推荐（重要）：
  - 针对不同抉择，尽量使用独立分支节点分别处理（例如 code-high/code-low）
  - 各分支节点尽量输出各自命名空间下的数据（避免同时写回同一个全局键）
  - 在分支末端增加一个"汇总计算节点"（通常是 code 节点）统一归一化输出
  - End 节点优先读取汇总节点结果，而不是直接读取分支节点结果
  - If-Else 后的汇总节点通常建议 `joinMode: "any"`（命中哪条分支就由哪条分支触发汇总）
        ...
        "data": {
          ...
          "config": {
            "outputs": [
              {
                "name": "decision",
                "source": "{{code-merge-1.summary.decision}}"
              },
              {
                "name": "score",
                "source": "{{code-merge-1.summary.score}}"
              },
              {
                "name": "level",
                "source": "{{code-merge-1.summary.level}}"
              }
            ]
          }
        }
